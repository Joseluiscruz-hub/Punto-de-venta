...
export async function authRoutes(app: FastifyInstance) {
  app.post(
    ...
    async (request, reply) => {
      ...
      if (!valid) {
        ...
        const lockedUntil =
          nextAttempts >= 5 ? new Date(Date.now() + 5 * 60_000).toISOString() : null;
        await database.query(
  ...
  app.post('/logout', async (request, reply) => {
    const token = request.cookies.el_triunfo_refresh;
    if (token)
      await database.query('UPDATE refresh_sessions SET revoked_at = now() WHERE token_hash = $1', [
        hashToken(token),
      ]);
