...
export default function DashboardView() {
  ...
  const categoryUtility = useMemo(() => products.slice(0, 5).map(p => ({ name: p.category, value: p.price - p.cost })), [products]);
    ...
    <div className="p-8 h-full overflow-y-auto custom-scrollbar space-y-8 bg-bg">
          ...
          <div className="h-[300px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={salesByHour}>
                ...
                <Tooltip contentStyle={{ backgroundColor: '#151922', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.08)', fontSize: '12px' }} />
      ...
      <SurfaceCard className="p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {['Verificar stock', 'Corte de caja', 'Ajustes', 'Revisar promociones'].map((task, index) => (
            <div key={index} className="rounded-3xl bg-surface-2 p-4 border border-border-subtle">
              <p className="text-sm font-black text-text-strong">{task}</p>
              <p className="text-[10px] text-text-secondary mt-2">Acción sugerida para el día.</p>
            </div>
          ))}
        </div>
      </SurfaceCard>
