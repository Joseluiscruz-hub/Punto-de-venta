...
import { printCashCut } from '../utils/printCashCut';

export function CorteCajaView({ onShiftClosed }: { onShiftClosed: () => void }) {
  const { reqContext, user, store } = useAuth();
  ...
  const handlePrintShift = async (shift: Shift, countedOverride?: number) => {
    ...
    try {
      ...
      printCashCut({
        ...
        countedCashOverride: countedOverride,
      });
    } catch (error) {
      alert(errorMessage(error, 'No se pudo generar el reporte de corte'));
    } finally {
      setPrintingId(null);
    }
  };
    ...
    <div className="view-shell p-4 sm:p-6 lg:p-8 h-full overflow-y-auto text-slate-900 dark:text-[#E2E8F0] flex flex-col gap-6 transition-colors">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 sm:gap-6">
            ...
            <div className="flex items-center gap-2">
              {activeShift && (
                <button
                  ...
                  disabled={printingId === activeShift.id}
                  className="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 dark:border-slate-700 px-2.5 py-1.5 text-[10px] font-bold uppercase tracking-wide hover:bg-slate-50 dark:hover:bg-slate-800 disabled:opacity-50"
                >
                  <Printer size={14} />
                  {printingId === activeShift.id ? 'Generando...' : 'Imprimir corte'}
                </button>
              )}
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-5 sm:p-6 shadow-sm">
            ...
            <p className="text-xs text-slate-500 leading-relaxed">El cierre de turno es una operación crítica. Asegúrate de contar todas las denominaciones antes de ingresar el monto final. Las diferencias mayores a {formatCurrency(activeShift?.differenceThreshold ?? 50)} quedarán confirmadas y auditadas. Usa <strong>Imprimir corte</strong> para el reporte físico del arqueo.</p>
          </div>
        </div>
      </div>
      <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl overflow-hidden flex flex-col shadow-sm">
        ...
        <div className="overflow-x-auto custom-scrollbar">
          <table className="w-full text-left text-[10px] sm:text-[11px] whitespace-nowrap min-w-[680px]">
            <thead className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-700 uppercase font-black tracking-[0.1em] text-slate-400 sticky top-0 z-10">
              <tr>
                ...
                <th className="px-4 sm:px-6 py-4">REPORTE</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {shifts.map((s) => (
                <tr key={s.id} className="hover:bg-primary/5 transition-colors">
                  ...
                  <td className="px-4 sm:px-6 py-4"><button type="button" onClick={() => void handlePrintShift(s)} disabled={printingId === s.id} className="inline-flex items-center gap-1 rounded-md border border-slate-200 dark:border-slate-700 px-2 py-1 text-[10px] font-bold uppercase hover:bg-slate-50 dark:hover:bg-slate-800 disabled:opacity-50" aria-label={`Imprimir corte ${s.id}`}><Printer size={12} />{printingId === s.id ? '...' : 'PDF/Print'}</button></td>
                </tr>
              ))}
