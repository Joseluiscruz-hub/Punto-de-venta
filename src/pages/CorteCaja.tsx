...
export function CorteCajaView({ onShiftClosed }: { onShiftClosed: () => void }) {
  ...
  const openPrintReport = async (shift: Shift) => {
    setPrintLoadingId(shift.id);
    try {
      const movements =
        activeShift?.id === shift.id
          ? cashMovements
          : await BackendAPI.getCashMovements(reqContext, shift.id);
      const counted = Number(countedCash);
      const reportShift: Shift =
        activeShift?.id === shift.id && Number.isFinite(counted) && countedCash !== ''
          ? {
              ...shift,
              actualCash: counted,
              difference: counted - shift.expectedCash,
            }
          : shift;
      setPrintMovements(movements);
      setPrintShift(reportShift);
      window.setTimeout(() => {
        const cleanup = () => {
      ...
      alert(errorMessage(error, 'No se pudo preparar el reporte de corte'));
    } finally {
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl xl:col-span-2 p-6 shadow-sm">
          ...
          {activeShift ? (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
              ...
              <div className="space-y-1">
                <p className="text-[10px] font-bold text-slate-400 uppercase">Cantidad de ventas</p>
                <p className="text-2xl font-mono font-bold text-slate-700 dark:text-slate-200">{activeShift.salesCount ?? 0}</p>
              </div>
              <div className="space-y-1">
              ...
              <div className="md:col-span-3 pt-6 border-t border-dashed border-slate-200 dark:border-slate-800 flex flex-col md:flex-row items-center justify-between gap-6">
                ...
                <div className="flex flex-col gap-3 w-full md:w-auto">
                  ...
                  <button onClick={handleClose} disabled={loading || !countedCash} className="w-full py-3 bg-rose-600 hover:bg-rose-700 text-white rounded-xl font-bold text-xs shadow-lg shadow-rose-500/20 transition-all active:scale-95 disabled:opacity-50">{loading ? 'Procesando...' : 'Cerrar Turno y Arqueo'}</button>
                  <button type="button" onClick={() => void openPrintReport(activeShift)} disabled={printLoadingId === activeShift.id} className="no-print w-full py-3 bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-bold text-xs transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2"><Printer size={14} />{printLoadingId === activeShift.id ? 'Preparando...' : 'Imprimir corte'}</button>
                </div>
              </div>
            </div>
          ) : (
        ...
        <div className="space-y-6">
          <form onSubmit={handleCashMovement} className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-5 sm:p-6 shadow-sm">
            ...
            <div className="mb-3 grid grid-cols-2 gap-2">
              <button type="button" onClick={() => setMovementType('CASH_IN')} className={`flex items-center justify-center gap-2 rounded-xl border px-3 py-2 text-xs font-bold ${movementType === 'CASH_IN' ? 'border-emerald-500 bg-emerald-50 text-emerald-700 dark:bg-emerald-950/40' : 'border-slate-200 dark:border-slate-700'}`}><ArrowDownCircle size={16} /> Entrada</button>
              <button type="button" onClick={() => setMovementType('CASH_OUT')} className={`flex items-center justify-center gap-2 rounded-xl border px-3 py-2 text-xs font-bold ${movementType === 'CASH_OUT' ? 'border-amber-500 bg-amber-50 text-amber-700 dark:bg-amber-950/40' : 'border-slate-200 dark:border-slate-700'}`}><ArrowUpCircle size={16} /> Retiro</button>
            </div>
            ...
            <button type="submit" disabled={movementLoading || !activeShift} className="btn-primary w-full py-3 text-xs disabled:opacity-50">{movementLoading ? 'Registrando...' : 'Registrar movimiento'}</button>
              ...
              <div className="mt-4 space-y-2 border-t border-slate-100 pt-4 dark:border-slate-800">
                {cashMovements.slice(0, 4).map((movement) => (
                  <div key={movement.id} className="flex items-start justify-between gap-3 text-xs">
                    ...
                    <strong className={movement.type === 'CASH_IN' ? 'text-emerald-600' : 'text-amber-600'}>{movement.type === 'CASH_IN' ? '+' : '-'}{formatCurrency(movement.amount)}</strong>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-5 sm:p-6 shadow-sm">
            <div className="flex items-center gap-3 mb-4"><Landmark className="text-primary-light" /><h4 className="font-bold text-xs uppercase">Resumen de Seguridad</h4></div>
            <p className="text-xs text-slate-500 leading-relaxed">El cierre de turno es una operación crítica. Asegúrate de contar todas las denominaciones antes de ingresar el monto final. Las diferencias mayores a{' '}{formatCurrency(activeShift?.differenceThreshold ?? 50)} quedarán confirmadas y auditadas.</p>
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-left text-[10px] sm:text-[11px] whitespace-nowrap min-w-[600px]">
            <thead className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-700 uppercase font-black tracking-[0.1em] text-slate-400 sticky top-0 z-10">
              <tr>
                ...
                <th className="px-4 sm:px-6 py-4 no-print">REPORTE</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {shifts.map((s) => (
                <tr key={s.id} className="hover:bg-primary/5 transition-colors">
                  <td className="px-4 sm:px-6 py-4"><p className="font-bold text-slate-900 dark:text-white">{s.endTime ? new Date(s.endTime).toLocaleString() : 'En curso'}</p><p className="text-[9px] text-slate-400 font-mono">{s.id}</p></td>
                  ...
                  <td className="px-4 sm:px-6 py-4"><span className={`px-2 py-1 rounded-[6px] text-[9px] font-black uppercase ${s.status === 'CLOSED' ? 'bg-slate-100 dark:bg-slate-800 text-slate-500' : 'bg-emerald-100 text-emerald-600'}`}>{s.status === 'CLOSED' ? 'Cerrado' : 'Abierto'}</span></td>
                  <td className="px-4 sm:px-6 py-4 no-print"><button type="button" onClick={() => void openPrintReport(s)} disabled={printLoadingId === s.id} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 px-2 py-1 text-[10px] font-bold text-slate-600 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300"><Printer size={12} />{printLoadingId === s.id ? '...' : 'Imprimir'}</button></td>
                </tr>
              ))}
              {shifts.length === 0 && (<tr><td colSpan={9} className="px-6 py-10 text-center text-slate-400 italic">No hay historial de turnos</td></tr>)}
            </tbody>
          </table>
        </div>
      </div>
